...
import { Trash2, CheckCircle, Mail, MessageSquare, User, Phone } from 'lucide-react'
          ...
          >
            ...
            >
              <div className="flex items-center gap-4 flex-1 min-w-0">
                {!isUnread && (
                  <span className="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg bg-stone-100 text-stone-600 font-bold text-xs border border-stone-200/60 shadow-sm">
                    {inquiries.length - index}
                  </span>
                )}
                <div className={`p-2 rounded-full shrink-0 ${isUnread ? 'bg-orange-100 text-orange-600' : 'bg-stone-100 text-stone-400'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 mt-1">
                    <p className={`text-sm truncate flex items-center gap-1.5 ${isUnread ? 'font-medium text-stone-800' : 'text-stone-500'}`}>

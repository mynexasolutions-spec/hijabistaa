...
export default function ProductFaqEditor({
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      await toggleUseGlobalFaqs(productId, useGlobal)
    ...
    <div className="bg-white rounded-xl border border-stone-200/80 p-6 space-y-4">
      ...
      <div className="pt-2 border-t border-stone-100 flex justify-end">
        <button
          ...
          onClick={handleSave}
          disabled={isPending}
          className="inline-flex items-center gap-2 px-6 py-2.5 bg-stone-900 text-white text-sm font-medium rounded-lg hover:bg-stone-800 disabled:opacity-60 transition-colors shadow-sm"
        >
          {isPending ? (
